...
def get_questions():
    ...
    questions_from_2d_array = questions.shuffle()
get_questions()
...
def Musical_genre():
    questions.set_file('musical_genre.txt')
    ...
    counter_reset()
def Musical_era():
    questions.set_file('musical_era.txt')
    questions.pack_questions_2d()
    questions.shuffle()
    get_questions()
    question_fun(counter)
...
def What_key():
    questions.set_file('What_key.txt')
    questions.pack_questions_2d()
    questions.shuffle()
    get_questions()
    question_fun(counter)
    ...
    print("time")
def Time_sigs():
    questions.set_file('time_sigs.txt')
    questions.pack_questions_2d()
    questions.shuffle()
    get_questions()
    question_fun(counter)
...
def play_music():
    ...
    song = questions_from_2d_array[counter][0].rstrip()
    playsound("file_example_MP3_700KB.mp3")

def mp3():
    questions.set_file('mp3_questions.txt')
    questions.pack_questions_2d()
    questions.shuffle()
    get_questions()
    counter_reset()
...
# sticky.grid(sticky=W)
Button(root, text="mp3", command=mp3).grid(row=1, column=12)
